...
async def test_fetch_one_returns_dict_or_none(tmp_db: DatabaseManager) -> None:
    ...
    row = await tmp_db.fetch_one(
        "SELECT key, ts FROM rate_limits WHERE key = ?",
        ("fetch_one",),
    )
...
async def test_fetch_all_returns_list_of_dicts(tmp_db: DatabaseManager) -> None:
    await tmp_db.execute("INSERT INTO rate_limits (key, ts) VALUES (?, ?)", ("a", 1.0))
    await tmp_db.execute("INSERT INTO rate_limits (key, ts) VALUES (?, ?)", ("b", 2.0))
...
async def test_failed_execute_raises_database_error(tmp_db: DatabaseManager) -> None:
    with pytest.raises(DatabaseError):
        await tmp_db.execute("INSERT INTO missing_table (value) VALUES (?)", ("x",))


@pytest.mark.asyncio
async def test_transaction_commits_on_success(tmp_db: DatabaseManager) -> None:
    async with tmp_db.transaction() as tx:
        await tx.execute("INSERT INTO rate_limits (key, ts) VALUES (?, ?)", ("tx_commit", 1.0))
...
@pytest.mark.asyncio
async def test_parallel_write_waits_for_open_transaction(tmp_db: DatabaseManager) -> None:
    async with tmp_db.transaction() as tx:
        await tx.execute("INSERT INTO rate_limits (key, ts) VALUES (?, ?)", ("inside", 1.0))
        outside = asyncio.create_task(
            tmp_db.execute("INSERT INTO rate_limits (key, ts) VALUES (?, ?)", ("outside", 2.0))
        )
