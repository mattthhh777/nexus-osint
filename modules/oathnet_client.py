"""
NexusOSINT — OathNet API Client  v3.0  (async)
Based on official OpenAPI spec at https://docs.oathnet.org/openapi.yaml

v3.0 changes (Phase 11 — cost optimization):
  ✅ Replaced sync `requests.Session` with async `httpx.AsyncClient`
  ✅ All methods converted to async — no asyncio.to_thread() needed
  ✅ TCP/TLS connection reuse across requests via httpx connection pool
  ✅ Module-level singleton: import `oathnet_client` from here
  ✅ Graceful error handling: httpx.HTTPStatusError / httpx.ConnectError / httpx.TimeoutException

Previous auth/endpoint corrections (preserved):
...
class BreachRecord:
    ...
    date:         str       = ""
    country:      str       = ""
    discord_id:   str       = ""   # captura discordid da API
    phone:        str       = ""   # captura phone/phone_number
    data_types:   list[str] = field(default_factory=list)
    extra_fields: dict      = field(default_factory=dict)  # campos extras não mapeados
...
class OathnetClient:
    """
    Async wrapper around the OathNet v2 REST API.
    Auth   : x-api-key header
    Base   : https://oathnet.org/api
    Client : persistent httpx.AsyncClient — TCP/TLS connection reuse, no per-request overhead.
    """

    def __init__(
        self,
        api_key: str,
        base_url: str = OATHNET_BASE_URL,
    ...
    async def _get(self, endpoint: str, params: dict | None = None) -> tuple[bool, dict]:
        url = endpoint.lstrip("/")
        try:
            ...
            logger.warning("OathNet timeout | endpoint=%s timeout=%ds", url, self.timeout)
            return False, {"error": f"Request timed out after {self.timeout}s — {url}"}
        except httpx.HTTPStatusError as exc:
            logger.warning("OathNet HTTP error | endpoint=%s status=%d", url, exc.response.status_code)
            return False, {"error": f"HTTP {exc.response.status_code}: {exc.response.text[:200]}"}

    async def _post(self, endpoint: str, payload: dict) -> tuple[bool, dict]:
        url = endpoint.lstrip("/")
        try:
    ...
    @staticmethod
    def _handle(resp: httpx.Response, url: str) -> tuple[bool, dict]:
    ...
    async def init_session(self, query: str) -> Optional[str]:
        ok, data = await self._post("service/search/init", {"query": query})
        if not ok:
    ...
    async def holehe(self, email: str, session_id: str = "") -> OathnetResult:
        result = OathnetResult(query=email, query_type="holehe")
        params: dict = {"email": email}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/holehe", params=params)
        if not ok:
    ...
    async def ip_info(self, ip: str, session_id: str = "") -> tuple[bool, dict]:
        params: dict = {"ip": ip}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/ip-info", params=params)
        return (True, data.get("data", data)) if ok else (False, {"error": data.get("error", "")})

    async def steam_lookup(self, steam_id: str, session_id: str = "") -> tuple[bool, dict]:
        params: dict = {"steam_id": steam_id}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/steam", params=params)
        return (True, data.get("data", data)) if ok else (False, {"error": data.get("error", "")})

    async def xbox_lookup(self, xbl_id: str, session_id: str = "") -> tuple[bool, dict]:
        params: dict = {"xbl_id": xbl_id}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/xbox", params=params)
        return (True, data.get("data", data)) if ok else (False, {"error": data.get("error", "")})

    async def roblox_lookup(self, username: str = "", user_id: str = "", session_id: str = "") -> tuple[bool, dict]:
        params: dict = {}
        if username:
        ...
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/roblox-userinfo", params=params)
        return (True, data.get("data", data)) if ok else (False, {"error": data.get("error", "")})

    async def discord_userinfo(self, discord_id: str, session_id: str = "") -> tuple[bool, dict]:
        params: dict = {"discord_id": discord_id}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/discord-userinfo", params=params)
        return (True, data.get("data", data)) if ok else (False, {"error": data.get("error", "")})

    async def discord_username_history(self, discord_id: str, session_id: str = "") -> tuple[bool, dict]:
        params: dict = {"discord_id": discord_id}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/discord-username-history", params=params)
        return (True, data.get("data", data)) if ok else (False, {"error": data.get("error", "")})

    async def extract_subdomains(self, domain: str, is_alive: bool = False, session_id: str = "") -> tuple[bool, dict]:
        params: dict = {"domain": domain, "is_alive": is_alive}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/extract-subdomain", params=params)
        return (True, data.get("data", data)) if ok else (False, {"error": data.get("error", "")})

    # ── Convenience wrappers ──────────────────────────────────────────────────

    async def search_email(self, email: str) -> OathnetResult:
        session_id     = await self.init_session(email) or ""
        breach         = await self.search_breach(email, session_id=session_id)
        stealer        = await self.search_stealer_v2(email, session_id=session_id)
        holehe_r       = await self.holehe(email, session_id=session_id)
    ...
    async def search_username(self, username: str) -> OathnetResult:
        session_id     = await self.init_session(username) or ""
        breach         = await self.search_breach(username, session_id=session_id)
        stealer        = await self.search_stealer_v2(username, session_id=session_id)
        breach.stealers       = stealer.stealers
        breach.stealers_found = stealer.stealers_found
        breach.session_id     = session_id
    ...
    async def validate_key(self) -> tuple[bool, str]:
        ok, data = await self._post("service/search/init", {"query": "nexusosint_keycheck"})
        if ok:
    ...
    async def ghunt(self, email: str, session_id: str = "") -> tuple[bool, dict]:
        """Google account OSINT via GHunt. May fail if OSID detection fails."""
        params: dict = {"email": email}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/ghunt", params=params)
        if not ok:
    ...
    async def minecraft_history(self, username: str, session_id: str = "") -> tuple[bool, dict]:
        """Minecraft username history via Mojang API proxy."""
        params: dict = {"username": username}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/mc-history", params=params)
        if not ok:
    ...
    async def victims_search(
        ...
        **filters: Any,
        ...
        for key in ("email", "ip", "hwid", "discord_id", "username"):
            val = filters.get(key)
            if val:
                params[f"{key}[]"] = val if isinstance(val, list) else [val]

        if filters.get("from_date"):
            params["from"] = filters["from_date"]
        if filters.get("to_date"):
            params["to"] = filters["to_date"]
        if filters.get("total_docs_min"):
            params["total_docs_min"] = filters["total_docs_min"]

        ok, data = await self._get("service/v2/victims/search", params=params)
        if not ok:
            return False, {"error": data.get("error", "Victims search failed.")}

        if "items" in data:
            return True, data
        return True, data.get("data", data)

    async def victims_manifest(self, log_id: str) -> tuple[bool, dict]:
        """Get file tree manifest for a victim log."""
        ok, data = await self._get(f"service/v2/victims/{log_id}")
        if not ok:
            return False, {"error": data.get("error", "Victim manifest not found.")}
        if "victim_tree" in data:
            return True, data
        return True, data.get("data", data)

    async def victims_file(self, log_id: str, file_id: str) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.warning("OathNet timeout | endpoint=%s", url)
            return False, f"Request timed out after {self.timeout}s"

    async def discord_to_roblox(self, discord_id: str, session_id: str = "") -> tuple[bool, dict]:
        """Find Roblox account linked to a Discord user ID."""
        params: dict = {"discord_id": discord_id}
        if session_id:
            params["search_id"] = session_id
        ok, data = await self._get("service/discord-to-roblox", params=params)
        if not ok:
            return False, {"error": data.get("error", "Discord→Roblox lookup failed.")}
        return True, data.get("data", data)


# ── Module-level singleton ────────────────────────────────────────────────────
# Import this in api/main.py: `from modules.oathnet_client import oathnet_client`
# Never instantiate OathnetClient per-request — TCP/TLS handshakes are expensive.

_api_key = os.getenv("OATHNET_API_KEY", "")
oathnet_client: OathnetClient | None = OathnetClient(api_key=_api_key) if _api_key else None
