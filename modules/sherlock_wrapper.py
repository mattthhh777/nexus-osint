...
async def _check_platform(client: httpx.AsyncClient, username: str, platform: dict) -> PlatformResult:
    ...
    try:
        ...
        if claim_type == "status_code":
            ...
            if claim_type == "text_present":
                ...
                result.found = text_lower not in body_lower and resp.status_code == 200
...
async def _run_async_checks(username: str) -> list[PlatformResult]:
    async with httpx.AsyncClient(
        ...
        tasks = [_check_platform(client, username, p) for p in PLATFORMS]
